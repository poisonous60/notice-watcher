...
class PollSection:
    ...
    # 다음 reprobe rc=0 / 정상 poll 가 cb=0 reset + 마커 unlink.
    # `breakage_threshold` 보다 살짝 위 — reprobe 가 1~2회 시도해도 안 풀리면 사용자에 알림.
    # `config.toml` `[poll] broken_threshold` 로 인스턴스별 override 가능.
