...
9. **N100 배포** (`docs/운영 메모.md` §8) —
   - 변경 파일을 `../notice-watcher/` 의 동일 경로로 `cp`: `configs/<slug>.json` (+ 손어댑터면 `adapters/<x>.py`·`adapters/__init__.py`; 인식기 추가했으면 `engine/known_platforms.py`; 엔진/스크립트 고쳤으면 그것도; `docs/사이트별 등록 시도 기록.md`).
