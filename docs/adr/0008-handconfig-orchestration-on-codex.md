...
## 병렬 위임 — worktree free-edit + diff-review 게이트 (2026-05-27 갱신)

`2026-05-21-fedi` batch(100 URL, 6 codex 청크)에서 병렬 위임을 검증했고, `2026-05-27-games-mobile` 진단에서 옛 파일 제한 방식이 Track B 일반화를 봉쇄하는 문제가 드러났다. **여러 codex 세션을 동시에 띄워 throughput 을 올리되, 안전은 worktree 격리 + Claude diff review 로 확보한다.**

**현재 결정**: hand-config/batch 위임은 `--worktree` 가 기본이다. codex 는 격리 branch 에서 필요한 repo 파일을 자유롭게 수정하고, Claude 가 `git diff main...codex-wt/<branch>` 를 읽어 merge 여부를 결정한다.

**규율 (Claude 가 entry 에서 강제)**:
1. **청크 = 분석 응집 단위** — 같은 플랫폼/host/cohort 신호를 묶어 cross-site pattern 을 보게 한다. 청크별 편집 파일 집합은 미리 정하지 않는다.
2. **Track B 파일 자유 편집** — `probe/`, `prompts/`, `generate/`, `scripts/register.py`, `engine/recognizers/`, `engine/strategies/` 등 필요한 repo 파일을 같은 worktree 안에서 함께 수정할 수 있다.
3. **diff-review = 진짜 게이트** — Claude 가 commit 전 `git diff main...branch` 로 변경 내용·scope·semantic 충돌·Track B 회피 여부를 직접 검증한다. 이게 실질 enforcement다.
4. **처방-우선 task 금지** — task 는 probe artifact 와 cohort 신호를 주는 분석 brief 여야 한다. 사이트별 처리 절차와 파일 제한을 미리 확정하면 codex 가 수동 config 실행기로 전락한다.
5. **auto-discovery semantic 충돌** — 새 recognizer 파일이 `recognize()` 전역 레지스트리에 영향 → 다른 플랫폼 URL 가로채 기존 테스트 깰 수 있음(fedi 에서 mbin PATTERN 이 XenForo `/threads` 가로챔). PATTERNS 를 **보수적**(고유 경로만)으로 + `probe_smoke --stage 5` 로 검출.
6. **공유 인덱스는 Claude 직렬** — INDEX.md·cases.sqlite3·git commit/push/배포는 병렬 codex 가 건드리면 레이스 → Claude 가 청크 수집 후 직렬 처리(`git add` 는 검토 통과 파일만 명시, `-A` 금지).

**진행 모델**: 첫 batch 는 *관측-우선*(1-2 청크 띄워 codex 품질 확인) → 신뢰되면 worktree 청크 다발 병렬. 다음 batch 부터 **병렬이 기본**(SKILL §0c).

**worktree 격리** — ✅ **구현됨** (commit 7c5a7f2): `codex_run.ps1 -Worktree` 가 codex 세션마다 분리 worktree+branch 생성 → edit 물리 격리 → same-tree race 0. Claude 가 branch review 후 merge. **다음 batch·다중 세션 동시 작업 시 `--worktree` 가 기본**.
...
- ✅ 병렬 위임 검증 완료 (2026-05-21-fedi): 6 청크 병렬/직렬, diff-review 게이트로 enforcement. 2026-05-27 부터 파일 제한 대신 worktree free-edit 모델로 갱신.
