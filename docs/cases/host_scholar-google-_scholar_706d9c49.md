...
## 무엇을 바꿨나 (fix layer: C+D+수동 config)
...
**수동 config** — `configs/host_scholar-google-_scholar_706d9c49.json`. httpx_html, article 섹션 생략. list.fields = post_id(data-aid), title, url, author, summary(`div.gs_rs` abstract). polite_sleep 15~30s (Google rate-limit 강함). `--config` path 는 `fetch_articles=0` 이라 body 검증 skip 통과.
...
수동 config 없이 자동 통과되는지는 다음 SERP/aggregator 사이트로 확인.
