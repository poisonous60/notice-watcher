...
status: ✅ 손-config (bbs-api-os getNewsList gids=8, 15건 baseline)
outcome: improved
date: 2026-05-19
fix_layer: F
...
차이: 첫 batch 에서 gen_fail posts_nonempty (probe 가 article cards 잡았지만 LLM 이 selector 박을 때 user profile 링크 추출 → posts_nonempty=0). 손-config 으로 우회.
