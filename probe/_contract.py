...
_ARTIFACTS: dict[str, ArtifactContract] = {
    ...
    "feed_candidates.json": ArtifactContract(
        ...
        list_item_fields={"candidates": (
            ...
            _ContractField("url"),
            _ContractField("validated", type_hint="bool",
                           note="후보 URL 을 fetch 해 non-empty RSS/Atom XML 로 확인했는지. path 추측/HTML/empty feed 는 false."),
            _ContractField("item_count", type_hint="int|null",
                           note="RSS item 또는 Atom entry 개수. XML feed 가 아니면 null, empty/fake feed 는 0."),
            _ContractField("content_type", type_hint="str|null"),
            _ContractField("root_tag", type_hint="'rss'|'feed'|'html'|null"),
